...
class ProfileController extends Controller
{
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'lanyard_image'        => 'nullable|image|mimes:jpeg,png,jpg,webp|max:5120',
        ...
        if ($request->hasFile('lanyard_image')) {
            $data['lanyard_image'] = $request->file('lanyard_image')->store('lanyard', 'public');
        }
    ...
    public function update(Request $request, Profile $profile)
    {
        ...
        if ($request->hasFile('lanyard_image')) {
            if ($profile->lanyard_image) {
                Storage::disk('public')->delete($profile->lanyard_image);
            }
            $data['lanyard_image'] = $request->file('lanyard_image')->store('lanyard', 'public');
        }
        if ($request->hasFile('cv_file')) {
